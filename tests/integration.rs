...
fn test_schwarzschild_properties_consistent() {
    ...
    let risco = schwarzschild_isco(M_SUN);
...
fn test_black_hole_thermodynamics_laws() {
    ...
    let s1 = bekenstein_hawking_entropy(M_SUN);
    let s2 = bekenstein_hawking_entropy(2.0 * M_SUN);
    assert!(s2 > s1);

    // Third law: temperature → 0 as mass → ∞
    let t1 = hawking_temperature(M_SUN);
    let t2 = hawking_temperature(1e6 * M_SUN);
    assert!(t2 < t1);
}
...
fn test_eddington_deflection_doubles_weak_field() {
    ...
    let r = 6.957e8;
    let d1 = light_deflection_weak_field(M_SUN, r);
    let d2 = light_deflection_weak_field(2.0 * M_SUN, r);
