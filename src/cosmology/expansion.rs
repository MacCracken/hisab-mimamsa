...
mod tests {
    ...
    fn test_comoving_distance_z1() {
        ...
        let d = comoving_distance(&params, 1.0, 1000).unwrap();
        let d_gpc = d / (GPC_M);
    ...
    fn test_angular_diameter_distance_turnover() {
        ...
        let params = CosmologicalParameters::planck2018();
        let d1 = angular_diameter_distance(&params, 1.0, 1000).unwrap();
        let d2 = angular_diameter_distance(&params, 1.6, 1000).unwrap();
        let d3 = angular_diameter_distance(&params, 3.0, 1000).unwrap();
    ...
    fn test_hubble_distance() {
        let params = CosmologicalParameters::planck2018();
        let dh = hubble_distance(&params).unwrap();
        let dh_gpc = dh / (GPC_M);
        // d_H ≈ 4.4 Gpc
    ...
    fn test_lookback_time_z1() {
        let params = CosmologicalParameters::planck2018();
        let t = lookback_time(&params, 1.0, 1000).unwrap();
        let t_gyr = t / GYR_S;
